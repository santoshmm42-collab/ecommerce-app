...
echo "<h1>🔧 Database Structure Analysis & Fix</h1>";
...
if ($constraints && $constraints->num_rows > 0) {
    echo "<table border='1' style='border-collapse: collapse; margin: 10px 0;'>";
    echo "<tr><th>Constraint</th><th>Table</th><th>Column</th><th>References</th><th>Ref Column</th></tr>";
    while($row = $constraints->fetch_assoc()) {
        echo "<tr><td>{$row['CONSTRAINT_NAME']}</td><td>{$row['TABLE_NAME']}</td><td>{$row['COLUMN_NAME']}</td><td>{$row['REFERENCED_TABLE_NAME']}</td><td>{$row['REFERENCED_COLUMN_NAME']}</td></tr>";
    }
    echo "</table>";
} else {
...
echo "<p>🗑️ Dropped existing tables</p>";
...
if ($conn->query($sql)) {
    ...
    echo "<p>❌ Error creating admin users table: " . $conn->error . "</p>";
}
...
foreach ($categories as $index => $cat) {
    ...
    if ($stmt->execute()) {
        $category_ids[$index + 1] = $conn->insert_id;
        echo "<p>✅ Category added: {$cat[0]} (ID: {$conn->insert_id})</p>";
    } else {
...
// Products array with correct category IDs
$products = [
...
if ($cat_count == 8 && $prod_count == 64) {
    echo "<h2>🎊 SUCCESS! Database setup completed successfully!</h2>";
    ...
    echo "<h2>⚠️ Some issues occurred. Please check the errors above.</h2>";
}
?>
